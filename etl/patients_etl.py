...
def run():
    ...
    with beam.Pipeline(options=options) as p:
        # Read data from the patients table
        patients_data = (
            p
            | 'ReadPatients' >> beam.Create([None])
            | 'FetchPatients' >> beam.ParDo(ReadPatients())
            | 'WriteToBigQuery' >> beam.io.WriteToBigQuery(
                'banded-lexicon-459415-q2:healthproject_dataset.patients',
