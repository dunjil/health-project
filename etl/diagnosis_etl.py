...
def run():
    ...
    with beam.Pipeline(options=options) as p:
        ...
        diagnosis_data = (
            p
            | 'ReadDiagnosis' >> beam.Create([None])
            | 'FetchDiagnosis' >> beam.ParDo(ReadDiagnosis())
            | 'WriteToBigQuery' >> beam.io.WriteToBigQuery(
                'banded-lexicon-459415-q2:healthproject_dataset.diagnoses',
